...
// Canonical production domain. Must match the domain the site is served from
// (connect nuansatranslator.co.id to the Vercel project for the sitemap to validate).
...
// Brand/location keywords shared across every page.
export const BASE_KEYWORDS = [
  ...
  "penerjemah bahasa Jakarta",
  "jasa penerjemah Jabodetabek",
  "Nuansa Translator",
];

// Builds a relevant keyword list for a service/industry page from its content.
// A page can override by setting `data.keywords`.
export function buildKeywords(data) {
  const list = [
    data.serviceName,
    data.serviceType,
    `${data.serviceType} Jakarta`,
    `${data.serviceType} Jabodetabek`,
    ...(data.types || []).slice(0, 4).map((t) => t.title),
    ...BASE_KEYWORDS,
  ].filter(Boolean);
  return [...new Set(list)];
}

// Full Next.js metadata (title, description, canonical, keywords, Open Graph, Twitter)
...
export function buildMetadata(data) {
  ...
  return {
    ...
    keywords: data.keywords || buildKeywords(data),
